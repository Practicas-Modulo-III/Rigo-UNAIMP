      ...
      {stands.map((shelf) => {
        ...
        return (
          <button
            ...
            className={
              'flex flex-col gap-1 rounded-lg border bg-slate-900 p-3 text-left transition-colors ' +
              (isActive ? 'border-rose-500 ring-1 ring-inset ring-rose-500/50' : 'border-slate-800 hover:border-slate-600')
            }
          >
            ...
            <p className="line-clamp-2 text-xs text-slate-400">{shelf.description}</p>
          </button>
        );
      })}
    </div>
